...
class RedGymEnv(Env):
    """Baseline Gym environment for Pokémon Red."""
    ...
    def __init__(self, config: Dict[str, Any] | None = None) -> None:
        ...
        self.release_arrow: List[WindowEvent] = [
        ...
        self.output_shape: Tuple[int, int, int] = (36, 40, 3)
        self.mem_padding: int = 2
        self.memory_height: int = 8
        self.col_steps: int = 16
        self.output_full: Tuple[int, int, int] = (
    ...
    def init_knn(self) -> None:
        """Initialize the k-NN index used for exploration bonuses."""
    ...
    def init_map_mem(self) -> None:
        """Reset the coordinate memory used for map exploration."""

        self.seen_coords: Dict[str, int] = {}

    def render(
        self, reduce_res: bool = True, add_memory: bool = True, update_mem: bool = True
    ) -> np.ndarray:
        """Render a frame and optionally update internal buffers."""
    ...
    def run_action_on_emulator(self, action: int) -> None:
        """Execute ``action`` on the emulator."""
    ...
    def add_video_frame(self) -> None:
        """Record the current frame to both video outputs."""
    ...
    def append_agent_stats(self, action: int) -> None:
        """Store environment statistics for analysis."""
    ...
    def update_frame_knn_index(self, frame_vec: np.ndarray) -> None:
        """Update the exploration k-NN index with the given frame."""
    ...
    def update_seen_coords(self) -> None:
        """Track coordinates visited for exploration reward."""
        x_pos = self.read_m(X_POS_ADDRESS)
        y_pos = self.read_m(Y_POS_ADDRESS)
        map_n = self.read_m(MAP_N_ADDRESS)
    ...
    def update_reward(self) -> Tuple[float, Tuple[float, float, float]]:
        """Compute shaped reward based on game progress."""
    ...
    def group_rewards(self) -> Tuple[float, float, float]:
        """Return decomposed reward components."""
    ...
    def create_exploration_memory(self) -> np.ndarray:
        """Visual memory representation of recent rewards."""
    ...
    def create_recent_memory(self) -> np.ndarray:
        """Return the recent reward memory buffer as an image."""
    ...
    def save_and_print_info(self, done: bool, obs_memory: np.ndarray) -> None:
        """Save screenshots and print reward information."""
    ...
    def read_bit(self, addr: int, bit: int) -> bool:
        """Return the bit value stored at ``addr``."""

        return bin(256 + self.read_m(addr))[-bit - 1] == "1"

    def get_levels_sum(self) -> int:
        """Return the total party level adjusted for the starter."""
    ...
    def get_levels_reward(self) -> float:
        """Return scaled reward value from the party levels."""
        explore_thresh = 22
        scale_factor = 4
    ...
    def get_knn_reward(self) -> float:
        """Reward for exploring novel states using k-NN distance."""
    ...
    def read_party(self) -> List[int]:
        """Return a list of Pokémon species IDs in the party."""

        return [self.read_m(addr) for addr in PARTY_ADDRESSES]

    def update_heal_reward(self) -> None:
        """Increase reward for healing and track deaths."""
        cur_health = self.read_hp_fraction()
        # if health increased and party size did not change
        if (
            cur_health > self.last_health
            and self.read_m(PARTY_SIZE_ADDRESS) == self.party_size
        ):
    ...
    def get_all_events_reward(self) -> int:
        """Count completed event flags for reward calculation."""
    ...
    def get_game_state_reward(self, print_stats: bool = False) -> Dict[str, float]:
        """Return individual reward terms derived from game memory."""
    ...
    def update_max_event_rew(self) -> int:
        """Track the maximum event reward value."""
        cur_rew = self.get_all_events_reward()
        self.max_event_rew = max(cur_rew, self.max_event_rew)
        return self.max_event_rew

    def read_hp_fraction(self) -> float:
        """Return the fraction of remaining party HP."""
    ...
    def read_hp(self, start: int) -> int:
        """Read a 16-bit HP value from memory."""
    ...
    def read_triple(self, start_add: int) -> int:
        """Read a 3-byte little-endian integer from memory."""
    ...
    def read_bcd(self, num: int) -> int:
        """Convert a BCD encoded byte into an integer."""

        return 10 * ((num >> 4) & 0x0F) + (num & 0x0F)

    def read_money(self) -> int:
        """Read the player's money amount."""
        return (
    ...
    def get_map_location(self, map_idx: int) -> str:
        """Return a human readable name for ``map_idx``."""
        map_locations = {
            0: "Pallet Town",
            1: "Viridian City",
            2: "Pewter City",
