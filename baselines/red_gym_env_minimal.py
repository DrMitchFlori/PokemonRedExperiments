...
class PokeRedEnv(Env):
    def __init__(
        ...
        """Create a minimal environment wrapper around PyBoy."""
    ...
    def reset(
        ...
        """Reset emulator state for a new episode."""
    ...
    def step(self, action: int) -> tuple[dict, float, bool, bool, dict]:
        """Advance the emulator by performing ``action``."""
    ...
    def run_action_on_emulator(self, action: int) -> None:
        """Execute ``action`` within the emulator."""
    ...
    def append_agent_stats(self, action: int) -> None:
        """Collect and store statistics from the current step."""
    ...
    def get_game_coords(self) -> tuple[int, int, int]:
        """Return the player's ``(x, y, map_index)`` coordinates."""
        return (self.read_m(0xD362), self.read_m(0xD361), self.read_m(0xD35E))

    def update_seen_coords(self) -> None:
        """Store the current coordinate visit time."""
        x_pos, y_pos, map_n = self.get_game_coords()
    ...
    def update_explore_map(self) -> None:
        """Mark the current global position as visited."""
    ...
    def get_explore_map(self) -> np.ndarray:
        """Return a cropped view of the exploration map."""
        c = self.get_global_coords()
        if c[0] >= self.explore_map.shape[0] or c[1] >= self.explore_map.shape[1]:
    ...
    def read_bit(self, addr: int, bit: int) -> bool:
        """Return the value of ``bit`` at ``addr``."""
    ...
    def get_levels_sum(self) -> int:
        """Return the total adjusted level of the party."""
        min_poke_level = 2
    ...
    def get_badges(self) -> int:
        """Return the number of badges obtained."""
        return self.bit_count(self.read_m(0xD356))

    def read_party(self) -> list[int]:
        """Return the Pokémon species IDs in the party."""
        return [
            self.read_m(addr)
    ...
    def get_all_events_reward(self) -> int:
        """Compute reward from all triggered event flags."""
    ...
    def update_max_op_level(self) -> int:
        """Track the highest opponent level seen so far."""
        opp_base_level = 5
        opponent_level = (
            max([
                self.read_m(a)
    ...
    def update_heal_reward(self) -> None:
        """Update counters related to healing events."""
    ...
    def update_map_progress(self) -> None:
        """Update maximum progress across the essential locations."""
    ...
    def get_map_progress(self, map_idx: int) -> int:
        """Return progress index for ``map_idx``."""
    ...
    def get_map_location(self, map_idx: int) -> dict:
        """Return the coordinates and name for ``map_idx``."""
        map_locations = {
