"""Gym environment for interacting with Pokemon Red via PyBoy."""
...
class RedGymEnv(Env):
    """Environment wrapper around PyBoy for reinforcement learning."""
    ...
    def __init__(self, config: dict | None = None) -> None:
        ...
        ----------
        config:
            Dictionary containing configuration values required to set up the
            environment.
        """
    ...
    def reset(self, seed: int | None = None, options: dict | None = None):
        ...
        ----------
        seed:
            Optional random seed for the environment.
        options:
            Optional dictionary of extra reset options.

        Returns
        -------
        tuple
            Observation and info dictionary as expected by Gymnasium.
        """
    ...
    def init_map_mem(self) -> None:
        """Initialize memory for visited map coordinates."""

        self.seen_coords: dict[str, int] = {}

    def render(self, reduce_res: bool = True) -> np.ndarray:
        """Return the current game screen as an array."""
    ...
    def step(self, action: int) -> tuple[dict, float, bool, bool, dict]:
        ...
        ----------
        action:
            Index of the action to perform from :attr:`valid_actions`.

        Returns
        -------
        Tuple containing observation, reward, terminated flag, truncated flag
        and info dictionary as defined by Gymnasium.
        """
    ...
    def run_action_on_emulator(self, action: int) -> None:
        """Send an action to the emulator and advance time."""
    ...
    def append_agent_stats(self, action: int) -> None:
        """Record environment statistics for debugging and analysis."""
    ...
    def start_video(self) -> None:
        """Initialise video writers for rollout recording."""
    ...
    def get_game_coords(self) -> tuple[int, int, int]:
        """Return in-game coordinates (x, y, map)."""
    ...
    def update_seen_coords(self) -> None:
        """Track all coordinates visited by the agent."""
    ...
    def get_current_coord_count_reward(self) -> int:
        """Return a small penalty for staying on the same tile."""
        x_pos, y_pos, map_n = self.get_game_coords()
        coord_string = f"x:{x_pos} y:{y_pos} m:{map_n}"
        if coord_string in self.seen_coords.keys():
    ...
    def get_global_coords(self) -> tuple[int, int]:
        """Return current coordinates in the global map space."""
        x_pos, y_pos, map_n = self.get_game_coords()
        return local_to_global(y_pos, x_pos, map_n)

    def update_explore_map(self) -> None:
        """Mark the agent's current position on the exploration map."""
    ...
    def get_explore_map(self) -> np.ndarray:
        """Return a local view of the global exploration map."""
        c = self.get_global_coords()
        if c[0] >= self.explore_map.shape[0] or c[1] >= self.explore_map.shape[1]:
    ...
    def update_recent_screens(self, cur_screen: np.ndarray) -> None:
        """Maintain a rolling buffer of recent screens."""

        self.recent_screens = np.roll(self.recent_screens, 1, axis=2)
        self.recent_screens[:, :, 0] = cur_screen[:, :, 0]

    def update_recent_actions(self, action: int) -> None:
        """Maintain a rolling history of recent actions."""

        self.recent_actions = np.roll(self.recent_actions, 1)
        self.recent_actions[0] = action

    def update_reward(self) -> float:
        """Update cumulative reward and return the delta for this step."""
        # compute reward
        self.progress_reward = self.get_game_state_reward()
        new_total = sum(
    ...
    def group_rewards(self) -> tuple[float, float, float]:
        """Return a tuple of reward components for logging."""
    ...
    def check_if_done(self) -> bool:
        """Return True if the episode has reached its step limit."""
    ...
    def save_and_print_info(self, done: bool, obs: dict) -> None:
        """Persist images and print debug info when episodes end."""
    ...
    def read_bit(self, addr: int, bit: int) -> bool:
        """Return the boolean value of a memory bit."""

        # add padding so zero will read '0b100000000' instead of '0b0'
        return bin(256 + self.read_m(addr))[-bit - 1] == "1"

    def read_event_bits(self) -> list[int]:
        """Return a list of all event flag bits."""
        return [
    ...
    def get_levels_reward(self) -> float:
        """Scaled reward based on party levels."""
        explore_thresh = 22
        scale_factor = 4
    ...
    def get_badges(self) -> int:
        """Return the number of badges acquired."""
    ...
    def get_game_state_reward(self, print_stats: bool = False) -> dict:
        """Compute the current reward components."""
    ...
    def update_max_event_rew(self) -> int:
        """Track the maximum event reward achieved."""
        cur_rew = self.get_all_events_reward()
        self.max_event_rew = max(cur_rew, self.max_event_rew)
        return self.max_event_rew

    def update_heal_reward(self) -> None:
        """Increase healing reward when party HP increases."""
    ...
    def read_hp_fraction(self) -> float:
        """Return current party HP as a fraction of max HP."""
        hp_sum = sum([
            self.read_hp(add)
    ...
    def fourier_encode(self, val: float) -> np.ndarray:
        """Return a Fourier encoding of a scalar value."""
        return np.sin(val * 2 ** np.arange(self.enc_freqs))
    
    def update_map_progress(self) -> None:
        """Update the furthest map index reached."""
