...
# Include all event flag addresses present in events.json
event_flags_end = 0xD887  # 0xD886 is the last valid address
...
class RedGymEnv(Env):
    def __init__(self, config=None):
        ...
        # Verify that generated keys match the event mapping
        for addr in range(event_flags_start, event_flags_end):
            for idx in range(8):
                key = f"0x{addr:X}-{idx}"
                if key not in event_names:
                    raise KeyError(f"Missing event key: {key} in events.json")
