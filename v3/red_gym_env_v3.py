...
class RedGymEnv(Env):
    ...
    def get_levels_sum(self):
        """Calculate the adjusted sum of the party's Pokémon levels.

        Levels below a minimum threshold are ignored and a starter bonus is
        subtracted from the result.

        Returns
        -------
        int
            Sum of all party levels after adjustments.
        """
        min_poke_level = 2
    ...
    def get_game_state_reward(self, print_stats=False):
        """Compute the reward components for the current game state.

        Parameters
        ----------
        print_stats : bool, optional
            Whether to print debugging information, by default False.

        Returns
        -------
        dict
            Mapping of reward component names to their current values.
        """
    ...
    def update_heal_reward(self):
        """Increase reward when the player's Pokémon regain health.

        If the party composition remains unchanged and the current health is
        higher than the previous value, reward is increased by the squared heal
        amount. A death increments the death counter instead.
        """
