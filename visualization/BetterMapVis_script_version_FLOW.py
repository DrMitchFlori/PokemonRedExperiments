...
def render_arrows(fname, all_flows, arrow_sprite):
    ...
    full_img = np.zeros(
        ((grid_dims[0] + 1) * cell_dim, (grid_dims[1] + 1) * cell_dim, 4),
        dtype=np.uint8,
    )
    for coord, total_move in tqdm(all_flows.items()):
        angle = math.atan2(-total_move[0], total_move[1])
        # mag = math.sqrt(coord[0]**2 + coord[1]**2)
        rotated_arrow = arrow_sprite.rotate(
            180 * angle / math.pi, resample=Image.Resampling.BICUBIC
        )
        nx = coord[0] - min_x
        ny = coord[1] - min_y
        ...
        full_img[
            nx * cell_dim : (nx + 1) * cell_dim, ny * cell_dim : (ny + 1) * cell_dim
...
def compute_flow_wrap(dat):
    ...
    return compute_flow(dat, inter_steps=1)
...
if __name__ == "__main__":
    ...
    arrow_size = 16  # 32
    arrow_img = Image.open("poke_map/transparent_arrow.png").resize(
