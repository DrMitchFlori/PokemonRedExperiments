...
def make_all_coords_arrays(filtered_dfs):
    return np.array(
        [tdf[["x", "y", "map"]].to_numpy().astype(np.uint8) for tdf in filtered_dfs]
    ).transpose(1, 0, 2)
...
def get_sprite_by_coords(img, x, y):
    ...
    return np.where(
        (sprite == alpha_v).all(axis=2).reshape(16, 16, 1),
        np.array([[[0, 0, 0, 0]]]),
        sprite,
    ).astype(np.uint8)
...
def render_video(fname, all_coords, walks, bg, inter_steps=4, add_start=True):
    ...
    errors = []
    sprites_rendered = 0
    turbo_map = get_cmap("cet_isoluminant_cgo_80_c38")._resample(
        8
    )  # mpl.colormaps['turbo']._resample(8)
        ...
        for idx in pbar:
            ...
            for fract in np.arange(0, 1, 1 / inter_steps):
                ...
                for run in range(len(step)):
                    ...
                    interp_coord = prev_p_coord + (
                        fract * (diff.astype(np.float32))
                    ).astype(np.int32)
                    ...
                    error = add_sprite(
                        over,
                        np.array(turbo_map(agent_version_float))
                        * walks[state[run]["dir"]],
                        interp_coord,
                    )
                ...
                pbar.set_description(
                    f"draws: {sprites_rendered} errors: {len(errors)}, {perc:.2%}"
                )
    return errors
...
def test_render(name, dat, walks, bg):
    ...
    return render_video(name, dat, walks, bg, inter_steps=8)


if __name__ == "__main__":

    run_dir = Path(
        "baselines/session_4da05e87_main_full"
    ...
    with Pool(procs) as p:
        run_steps = 16385
        base_data = rearrange(
            base_coords, "(v s) r c -> s (v r) c", v=base_coords.shape[0] // run_steps
        )
